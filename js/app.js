...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <h3 class="product-titile">${product.title}</h3>
      <p>Category: ${product.category}</p>
      <p>Rating: <span class="rating">${product.rating.rate}</span></p>
      <p>Total Rating Count: <span class="rating">${product.rating.count}</span></p>
      <h2>Price: $ ${product.price}</h2>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success">add to cart<i class="fas fa-cart-plus"></i></button>
      <button id="details-btn" class="btn btn-danger">Details<i class="fas fa-info-circle"></i></button></div>
      `;
...
const updatePrice = (id, value) => {
  ...
  // format price with two decimals fixed
